...
public class WeekView extends View {
    ...
    private EmptyViewMonthChangeListner onMonthChangeListner;
    ...
    private void drawHeaderRowAndEvents(Canvas canvas) {
        ...
        // customise by vikash for view hide from header  9/4/2019 line no 490 comment
        //  canvas.drawRect(0, 0, getWidth(), mHeaderTextHeight + mHeaderRowPadding * 2, mHeaderBackgroundPaint);

        // Draw the header row texts.
        // customise by vikash for view hide from header  9/4/2019 line no 494 comment
        //startPixel = startFromPixel;

        // customise by vikash for view hide from header  9/4/2019 line 490 to 509
    ...
    private void getMoreEvents(Calendar day) {
        ...
        if (mFetchedMonths[0] < 1 || mFetchedMonths[0] != previousMonth || mRefreshEvents) {
            if (!containsValue(lastFetchedMonth, previousMonth) && !isInEditMode()) {
                List<WeekViewEvent> events = mMonthChangeListener.onMonthChange((previousMonth == 12) ? day.get(Calendar.YEAR) - 1 : day.get(Calendar.YEAR), previousMonth, day);
                sortEvents(events);
        ...
        if (mFetchedMonths[1] < 1 || mFetchedMonths[1] != day.get(Calendar.MONTH) + 1 || mRefreshEvents) {
            if (!containsValue(lastFetchedMonth, day.get(Calendar.MONTH) + 1) && !isInEditMode()) {
                List<WeekViewEvent> events = mMonthChangeListener.onMonthChange(day.get(Calendar.YEAR), day.get(Calendar.MONTH) + 1, day);
                sortEvents(events);
                for (WeekViewEvent event : events) {
                    cacheEvent(event);
                }
        ...
        if (mFetchedMonths[2] < 1 || mFetchedMonths[2] != nextMonth || mRefreshEvents) {
            if (!containsValue(lastFetchedMonth, nextMonth) && !isInEditMode()) {
                List<WeekViewEvent> events = mMonthChangeListener.onMonthChange(nextMonth == 1 ? day.get(Calendar.YEAR) + 1 : day.get(Calendar.YEAR), nextMonth, day);
                sortEvents(events);
                for (WeekViewEvent event : events) {
                    cacheEvent(event);
                }
    ...
    public interface MonthChangeListener {
        List<WeekViewEvent> onMonthChange(int newYear, int newMonth, Calendar time);
    }
    ...
    public interface EmptyViewMonthChangeListner {
        void onMonthChangeListner(Calendar time);
    }
