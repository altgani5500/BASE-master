...
import com.cancan.Adapters.MyFragmentPagerAdapter
...
class ActivityJobsPreview : BaseActivity() {
    ...
    private fun getIntentContent() {
        fileList = intent.getSerializableExtra("FILE_LIST") as ArrayList<File>
        adapter = MyFragmentPagerAdapter(this@ActivityJobsPreview, fileList)
        ...
        prev_totalHrPerWeek.text = getWorkingHoursFormJonTotal(intent.getSerializableExtra("WORKING_HOUR").toString())
    ...
    private fun getWorkingHoursFormJonTotal(json: String): String {
        ...
        if(json.trim().contentEquals("N")){
            return "0"
        }
        if(json.trim().equals("N")){
            return "0"
        }
    ...
    private fun getWorkingHoursFormJon(json: String): String {
        var finalString = StringBuilder()
        if (json.contentEquals("1")) {
            finalString.append(getString(R.string.flexible))
        } else {
            val jsonArry = JSONArray(json)
            for (i: Int in 0 until jsonArry.length()) {
                val item = jsonArry.optJSONObject(i)
